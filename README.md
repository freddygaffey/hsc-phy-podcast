# hsc-phy-podcast — Lecture Audio

A standalone PWA that turns the HSC Physics syllabus into spoken lecture
recordings, fully local to generate — no cloud, no API keys. Deployed on Cloudflare
Pages (app) + R2 (audio); see **`DEPLOY.md`**.

> **Content source.** The lecture content is derived from the NSW HSC Physics syllabus
> and reputable physics sources.
...
content/
  M7-05-The-Photoelectric-Effect/
    ...
    supplementary.md   # equations + worked examples shown to the user (never spoken)
...
Case studies follow the same shape but use a `case_descriptive_title/` folder name
(e.g. `case_ligo_detects_gravitational_waves/`) with the same `script.md` +
`supplementary.md` structure inside.

- `content/<episode>/script.md` — the spoken script. YAML frontmatter + the lecture body.
- `content/<episode>/supplementary.md` — read-along equations/worked examples, rendered to the user, never narrated.
...
./generate_all_voices.sh M7-05-The-Photoelectric-Effect   # render just one episode (folder name)
```
